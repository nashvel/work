<x-app-layout>
    @php
        $company_info = App\Models\Contact::where('id', $id)->first();
    @endphp
    <x-slot name="title">{{ $company_info->company_name }}</x-slot>
    <x-slot name="url_1">{"link": "/contact/list", "text": "Relationship"}</x-slot>
    <x-slot name="url_2">{"link": "/contact/list", "text": "Manage"}</x-slot>
    <x-slot name="active">{{ $company_info->company_name }}</x-slot>
    <x-slot name="buttons">
        <a href="/relationship/{{ $id }}/person/new" class="ti-btn ti-btn-primary !border-0 btn-wave me-0">
            <i class="bi bi-plus-lg me-1"></i> Register Contact
        </a>
    </x-slot>
            ...
            <div
                ...
                <div class="max-h-full w-full overflow-hidden ti-modal-content">
                    <div class="ti-modal-header">
                        <h6 class="modal-title text-[1rem] font-semiboldmodal-title" id="modal-form-title">List</h6>
    ...
    @include('pages.contacts.list_person')
</x-app-layout>
