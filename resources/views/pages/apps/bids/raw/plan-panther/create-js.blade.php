 @php
     ...
     $clients = App\Models\ContactPerson::with(['company:id,company_name,city,state,lead_source'])
         ->whereHas('company', function ($query) {
             ...
             $query->orWhere('lead_source', 'Potential Plan Panther Subscription');
         })
 ...
 <script>
     $(document).ready(function() {
         ...
         window.addStage = function() {
             ...
             stageBlock.innerHTML = `
                <div class="flex justify-between items-center">
                    <strong class="text-2xl">${selectedStage}</strong>
                ...
                <input class="form-control" placeholder="Write Subject here.." name="stage_subject[${sanitizedStage}]"/>
                ...
                <select class="form-control invite-client" name="invite_clients[${selectedStage}][]" multiple>
                    <option value="bidstmc@gmail.com" selected>Bids TMC</option>
                    @foreach ($clients as $client)
                        <option value="{{ $client->email }}" {{ $client->id == 4507 ?? 'selected' }}>
                            {{ $client->first_name }} {{ $client->last_name }} ({{ $client->company->company_name }})
                        </option>
                    @endforeach
                </select>
             `;
         ...
         window.removeStage = function(stage) {
             ...
             document.getElementById('proj_stage').querySelector(`option[value="${stage}"]`).disabled =
                 false;
         }
         ...
         function initializeQuillEditor(editorId, hiddenInputId) {
            ...
            //  const contentInput = document.querySelector('#project_description');
