...
<body class="bg-gray-100 font-[Rubik]">
    ...
    <div id="chat-panel"
        ...
        <div class="flex">
            ...
            <!-- Chat Section -->
            <main class="flex-1 bg-white flex flex-col rounded-lg overflow-hidden" style="height: 600px;">
                <!-- Chat Header -->
                ...
                <div id="chat-list" class="flex-1 p-4 space-y-2 overflow-y-auto bg-gray-50">
                    ...
                    <div id="incoming-call"
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm">
                                A customer has initiated a call.
                                <a id="incoming-call-link" href="#" target="_blank"
                                    class="underline font-medium text-white hover:text-blue-100">
                                    Join Now
                                </a>
                            </p>
        ...
        <script type="module">
            ...
            function loadConversations() {
                ...
                if (data.type === 'call' && data.status === 'started' && !incomingCallActive) {
                    const startTime = data.started_at?.toDate?.();
                    const now = new Date();

                    if (startTime && (now - startTime) / 1000 < 60) {
                        showIncomingCall(data.link);
                    }
                }

                if (data.type === 'call' && data.status === 'cancelled') {
                    dismissIncomingCall();
                }
            }
            ...
            async function openConversation(customerId) {
                ...
                unsubscribe = onSnapshot(q, snapshot => {
                    snapshot.docChanges().forEach(change => {
                        ...
                        // Auto cancel after 60 seconds (server time)
                        if (
                            data.type === 'call' &&
                            data.status === 'started' &&
                            ...
                            if (diffSeconds >= 60) {
                                // Check if no existing cancel message yet
                                const cancelMsg = snapshot.docs.find(d => {
                                    const dData = d.data();
                                    return dData.type === 'call' && dData.status === 'cancelled' &&
                                        dData.timestamp;
                                });
                            }
                        }
        ...
        `;
            ...
            async function startCall(type) {
                ...
                const callMessage =
                    `🔔 ${callType} started: <a href="${callLink}" target="_blank" class="text-blue-600 underline">Join Now</a>`;
    ...
    <script>
        ...
        function showIncomingCall(link) {
            ...
            const callLink = document.querySelectorAll('#incoming-call-link');

            callLink.forEach(el => el.href = link);
